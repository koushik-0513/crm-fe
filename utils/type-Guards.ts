...
export const isApiErrorResponse = (data: unknown): data is {
  ...
  return Boolean(data && typeof data === 'object' && 'error' in data && 'success' in data && data.success === false);
};
...
export const hasSuccess = (data: unknown): data is { success: boolean } => {
  return isRecord(data) && 'success' in data && typeof (data as Record<string, unknown>).success === 'boolean';
};
...
export const isValidApiResponse = (data: unknown): data is {
  ...
  return isRecord(data) && hasSuccess(data) && (hasData(data) || hasMessage(data));
};
