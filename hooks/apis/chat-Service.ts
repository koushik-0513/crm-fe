...
import { validateApiResponse, chatDeleteResponseSchema, chatTitleUpdateResponseSchema } from "@/types/validation-Schemas";
...
export const deleteConversation = async (conversationId: string): Promise<TChatDeleteResponse> => {
  ...
  const data = await response.json();
  return validateApiResponse(chatDeleteResponseSchema, data);
};
...
export const updateConversationTitle = async (
  ...
  const data = await response.json();
  return validateApiResponse(chatTitleUpdateResponseSchema, data);
};
